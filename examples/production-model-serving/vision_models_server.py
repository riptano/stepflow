...
try:
    ...
    print("Please install stepflow-py or run from the project root with proper PYTHONPATH")
    SDK_AVAILABLE = False
...
try:
    ...
    CV_AVAILABLE = False
    logger.warning("Computer vision libraries not available, using mock implementations")
...
def get_or_load_vision_model(model_name: str):
    ...
    try:
        ...
        device = 0 if torch.cuda.is_available() and config['resource_tier'] == 'gpu' else -1
...
async def classify_image(input: ImageClassificationInput) -> ImageClassificationOutput:
    ...
    try:
        ...
        if not CV_AVAILABLE:
            ...
            # Format results
            predictions = sorted(results, key=lambda x: x['score'], reverse=True)[:input.top_k]
...
async def batch_classify_images(input: BatchImageInput) -> BatchImageOutput:
    ...
    try:
        ...
        avg_inference_time = total_inference_time / len(input.images) if len(input.images) > 0 else 0
    ...
    try:
        if not CV_AVAILABLE:
            ...
            return ImageMetricsOutput(
                ...
                format="JPEG",
                mode="RGB",
                size_bytes=len(input.image_data) * 3 // 4,  # Approximate base64 decode size
                recommended_model="deit-efficient",
                preprocessing_required=False
            )
        
        image = decode_image(input.image_data)
        size_bytes = len(base64.b64decode(input.image_data.split(',')[1] if ',' in input.image_data else input.image_data))
    ...
    if input.include_model_details:
        ...
        for model_name, config in VISION_MODEL_REGISTRY.items():
            model_details[model_name] = {
                ...
                "memory_estimate_mb": 100 if config['resource_tier'] == 'cpu' else 500,  # Rough estimates
            }
    
    return VisionHealthOutput(
        status="healthy",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", type=str, default="localhost", help="Host for HTTP mode")
    args = parser.parse_args()
    
    logger.info("Starting Vision Models Server")
    logger.info(f"Available models: {list(VISION_MODEL_REGISTRY.keys())}")
    logger.info(f"GPU available: {CV_AVAILABLE and torch.cuda.is_available() if CV_AVAILABLE else False}")
    ...
    if args.http:
        ...
        http_server = StepflowHttpServer(host=args.host, port=args.port)
        
        # Register components on HTTP server
        http_server.server.component(vision_health_check)
        http_server.server.component(classify_image)
        http_server.server.component(analyze_image_metrics)
